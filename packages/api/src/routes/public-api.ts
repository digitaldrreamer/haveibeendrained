...
/**
 * GET /api/v1/check - Unified endpoint for checking drainer addresses and wallet security
 * 
 * Flow:
 * 1. Check if address is a known drainer (on-chain registry)
 * 2. If not a drainer, perform full wallet analysis
 * 
 * Rate limiting: Based on User-Agent format
...
app.get('/check',
  // Middleware chain
  userAgentParser(),                    // Parse and validate User-Agent
  cacheMiddleware(),                     // Check cache, bypass rate limits for cached
  createPublicApiRateLimiter(),         // Apply rate limiting based on tier
...
/**
 * GET /api/v1/drainer/:address - Check only if address is a known drainer (faster)
 * Useful for quick drainer checks without full analysis
 */
 ...
 * Useful when you know the address is not a drainer
 */
