...
const walletConnectAdapter = new WalletConnectWalletAdapter({
  ...
  options: {
    ...
    metadata: {
      ...
      url: 'https://haveibeendrained.com',
      icons: ['https://haveibeendrained.com/logo.png'],
    },
  },
});
...
- ✅ Works on both mobile browser and native apps
- ✅ No private key exposure
- ✅ Persistent session across browser refresh
- ✅ Support for complex multi-step flows
...
⚠️ **Status:** Viable for simple operations only, NOT recommended for complex signing flows.
...
1. **❌ Using localStorage for session tokens** → Use secure httpOnly cookies instead
2. **❌ Not simulating transactions** → Always simulate before asking user to sign
3. **❌ Reusing old blockhashes** → Fresh blockhash required (expires after 150 blocks / ~60-90 seconds)
4. **❌ Deep linking on mobile** → Use WalletConnect instead for smooth UX
5. **❌ Ignoring nonce in message signing** → Critical for preventing replay attacks
6. **❌ Not handling wallet disconnection** → Users can disconnect mid-flow

### 🎯 For Your "Have I Been Drained" Project
