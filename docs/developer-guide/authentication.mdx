...
```bash
curl "https://api.haveibeendrained.org/api/v1/check?address=..."
```
...
<Info>
  API keys are available for enterprise tier users. Contact support@haveibeendrained.org to request an API key.
</Info>
...
**Process:**
1. Email support@haveibeendrained.org
...
const response = await fetch(
  'https://api.haveibeendrained.org/api/v1/check?address=...',
  {
    headers: {
      'User-Agent': 'MyApp/1.0.0 (contact@example.com)',
